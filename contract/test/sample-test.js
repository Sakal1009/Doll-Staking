...
describe("DollNFTStaking.sol", () => {
    ...
    beforeEach(async () => {
        ...
        // Deploy DollFT ERC20 token contract 
    ...
    describe("Correct Deployement", () => {
        ...
        it("NFT contract should have correct initial parameters", async () => {
            ...
            await expect(nftContract.tokenURI(1)).to.be.revertedWithCustomError(nftContract, 'dollQueryForNonExistentToken');
        });
    ...
    describe("Core Functions", () => {
        ...
        it("should allow user to unstake his tokens", async () => {
            ...
            expect(Math.floor(getAmountFromWei(user1_tokenBalance))).to.equal(720);
        ...
        it("should calculate correct daily reward based on staking period", async () => {
            ...
            const lessThanTwentyMonthPeriod = 210 * 24 * 60 * 60;
            const moreThanTwentyMonthPeriod = 390 * 24 * 60 * 60;
            ...
            expect(await stakingVault.getDailyReward(lessThanSixMonthPeriod)).to.equal(3);
            expect(await stakingVault.getDailyReward(lessThanTwentyMonthPeriod)).to.equal(4);
            expect(await stakingVault.getDailyReward(moreThanTwentyMonthPeriod)).to.equal(8);
        });
        ...
        it("should not allow user to mint NFT while contract is paused", async () => {
            ...
            await expect(
                ...
                nftContract, "dollContractIsPaused"
            )
        });
        ...
        it("should not allow not NFT owner to stake his nfts", async () => {
            ...
            await expect(stakingVault.connect(user2).stake(tokenIds)).to.be.revertedWithCustomError(stakingVault, "dollNFTStakingVaultNotItemOwner")
        });
        ...
        it("should not allow user to stake same NFT twice", async () => {
            ...
            await expect(stakingVault.connect(user1).stake([0])).to.be.revertedWithCustomError(stakingVault, "dollNFTStakingVaultItemAlreadyStaked")
        });
        ...
        it("should not allow not NFT owner to claim staking reward", async () => {
            ...
            await expect(stakingVault.connect(user2).claim(tokenIds)).to.be.revertedWithCustomError(stakingVault, "dollNFTStakingVaultNotItemOwner")
        });
        ...
        it("should not allow not NFT owner to unstake others NFTs", async () => {
            ...
            await expect(stakingVault.connect(user2).unstake(tokenIds)).to.be.revertedWithCustomError(stakingVault, "dollNFTStakingVaultNotItemOwner")
        });
        ...
        it("should not allow random users to mint KPT tokens", async () => {
            ...
            ).to.be.revertedWithCustomError(tokenContract, "dollFTOnlyControllersCanMint")
        });
    });
